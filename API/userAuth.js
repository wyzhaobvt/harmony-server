...
app.post("/registerUser", async function (req, res) {
  try {
    ...
    await req.db.query(
      "INSERT INTO users (email, password, username , userCallLink , deleted) VALUES (:email, :password, :email , :calllink , false)",
      {
        email: user.email,
...
app.post("/loginUser", async function (req, res) {
  try {
    ...
    const accessToken = jwt.sign(
      { email: user.email, securePassword: user.password },
      process.env.JWT_KEY
    );
